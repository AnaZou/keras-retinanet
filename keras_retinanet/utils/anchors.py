...
def anchor_targets(
    ...
    **kwargs
    ...
    anchors, mask = anchors_for_shape(image_shape, mask_shape, **kwargs)
...
def anchors_for_shape(
    ...
    mask_shape  = np.array(mask_shape[:2])
    for i in range(pyramid_levels[0] - 1):
        image_shape = (image_shape + 1) // 2
        mask_shape  = (mask_shape + 1) // 2

    # compute anchors over all pyramid levels
    all_mask = np.zeros((0,), dtype=int)
    ...
    for idx, p in enumerate(pyramid_levels):
        ...
        mask = np.ones(tuple(image_shape) + (len(anchors),), dtype=all_mask.dtype)
        mask[:mask_shape[0], :mask_shape[1], :] = 0
        all_mask = np.append(all_mask, mask.ravel(), axis=0)

    return all_anchors, all_mask
